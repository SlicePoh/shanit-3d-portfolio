...
export function ItemContent({ item }: { item: PortfolioItem }) {
  return <>
    ...
    {item.kind === "certification" && <div className="project-links">{item.link ? <a href={item.link} target="_blank" rel="noopener noreferrer">View credential <ArrowUpRight size={14} /></a> : <p className="missing-link">Credential URL not supplied.</p>}</div>}
...
export default function ProjectPanel({ item }: { item: PortfolioItem }) {
  return <>
    ...
    <div className="project-links">
      {item.github ? <a href={item.github} target="_blank" rel="noopener noreferrer"><Github size={15} /> GitHub <ArrowUpRight size={14} /></a> : <button disabled title="The repository URL has not been provided"><Github size={15} /> GitHub <span>not supplied</span></button>}
      {item.link ? <a href={item.link} target="_blank" rel="noopener noreferrer">Live demo <ArrowUpRight size={14} /></a> : <button disabled title="The live demo URL has not been provided">Live demo <span>not supplied</span></button>}
    </div>
  </>;
}