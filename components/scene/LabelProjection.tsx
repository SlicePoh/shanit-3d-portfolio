...
export default function LabelProjection() {
  const markers = useMemo(() => stores.map((store) => ({
    ...
    world: new Vector3(store.scale[0] / 2 + 0.08, store.scale[1] * 0.62, store.scale[2] / 2).applyEuler(new Euler(...store.rotation)).add(new Vector3(...store.position)),
  ...
  useFrame(({ camera, size }) => {
    for (const marker of markers) {
      ...
      const x = Math.max(6, Math.min(size.width - element.offsetWidth - 6, (marker.projected.x * 0.5 + 0.5) * size.width));
